...
async def process_ball_result(client, match_id, chat_id, batter_choice, bowler_choice,
    ...
    if is_out:
        ...
        # Send image with caption for six/four, otherwise just text
        if runs == 6:
            await send_image_with_caption(client, chat_id, "six", result_text)
        elif runs == 4:
            await send_image_with_caption(client, chat_id, "four", result_text)
        else:
...
async def update_solo_state(client, match_id, chat_id, runs, is_out, ball_num):
    ...
    if batter_done:
        ...
        await safe_send(client, chat_id,
            ...
            f"🎳 {mention(bowler_uid, bwname)}, you're bowling! Check my DM for your turn!")
...
async def finish_solo_match(client, match_id, chat_id, scores, player_ids):
    ...
    # Send win image with scoreboard as caption
    await send_image_with_caption(client, chat_id, "win", "\n".join(lines))
    cancel_timer(f"{match_id}_batter")
    cancel_timer(f"{match_id}_bowler")
...
async def handle_over_change_team(client, match_id, chat_id, batter_id, non_striker_id):
    ...
    await safe_send(client, chat_id,
        ...
        f"🎳 {mention(new_bowler_id, bwname)}, you're bowling! Check my DM!")
...
async def start_solo_match(client, match_id, chat_id):
    ...
    await safe_send(client, chat_id,
        ...
        f"🏏 {mention(first_batter, bname)}, you're batting! Type **1-6** in the group!\n"
        f"🎳 {mention(bowler_uid, bwname)}, you're bowling! Check my DM for your turn!")
