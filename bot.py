...
# ═══════════════════════════════════════════════
# AFK TIMEOUT WITH WARNINGS
# ═══════════════════════════════════════════════

async def start_afk_timer(client, match_id, target_type, target_user_id, chat_id):
    """Start an AFK timer with warnings. target_type is 'batter' or 'bowler'."""
    ...
    async def _timeout():
        ...
        name = safe_name(user) if user else "Player"
        role = "bowl" if target_type == "bowler" else "bat"
        ...
        if not match or match["state"] == MatchState.MATCH_OVER:
            return
        expected_state = MatchState.BOWLER_WAITING if target_type == "bowler" else MatchState.BATTER_WAITING
        if match["state"] == expected_state:
            await safe_send(client, chat_id,
                f"⏳ {mention(target_user_id, name)}, **50 seconds** left to {role}! Hurry up! 🏃")
        ...
        if match["state"] == expected_state:
            await safe_send(client, chat_id,
                f"⚠️ {mention(target_user_id, name)}, only **20 seconds** left! Play NOW or timeout! ⏱🔥")
        
        # Final timeout at 80 seconds
        ...
        if target_type == "bowler":
            ...
            if match["current_bowler_id"] != target_user_id:
                return
            # Bowler AFK -> Eliminate from game, no penalty
            await safe_send(client, chat_id,
                f"⏱ **AFK TIMEOUT!** {mention(target_user_id, name)} (bowler) didn't respond!\n"
                f"🚫 **ELIMINATED** from the match!")
            await handle_afk_bowler(client, match_id, chat_id, target_user_id)
...
async def handle_afk_bowler(client, match_id, chat_id, afk_bowler_id):
    """Handle AFK bowler - eliminate from game, no penalty runs."""
    match = Match.get_match_by_id(match_id)
    if not match:
        return
    ...
    if mode == "solo":
        # Remove AFK player from batting order so they don't get a turn
        player_ids = Match.get_team_batting_order(match)
        player_ids_str = [str(uid) for uid in player_ids]
        
        # Remove AFK bowler from order
        if str(afk_bowler_id) in player_ids_str:
            player_ids_str.remove(str(afk_bowler_id))
            Match.set_team_batting_order(match_id, [int(x) for x in player_ids_str])
            
            # Fix solo_batting_index if needed
            batter_str = str(batter_id)
            if batter_str in player_ids_str:
                new_bat_idx = player_ids_str.index(batter_str)
                Match.update_match(match_id, solo_batting_index=new_bat_idx)
        
        # Also mark their solo score as eliminated
        scores = Match.get_solo_scores(match)
        afk_str = str(afk_bowler_id)
        if afk_str in scores:
            scores[afk_str]["is_out"] = True
            scores[afk_str]["batting"] = False
        else:
            scores[afk_str] = {"runs": 0, "balls": 0, "is_out": True, "batting": False}
        Match.set_solo_scores(match_id, scores)
        
        # Check if enough players left
        if len(player_ids_str) < 2:
            # Not enough players - end match
            await safe_send(client, chat_id, "❌ Not enough players left! Match ended.")
            await finish_solo_match(client, match_id, chat_id, scores, player_ids_str)
            return
        
        # Find next bowler (not batter, not eliminated)
        batter_idx = player_ids_str.index(str(batter_id)) if str(batter_id) in player_ids_str else 0
        current_bowl_idx = match["solo_bowling_index"]
        
        # Adjust bowl index since list changed
        new_bowl_idx = current_bowl_idx % len(player_ids_str)
        if int(player_ids_str[new_bowl_idx]) == batter_id:
            new_bowl_idx = (new_bowl_idx + 1) % len(player_ids_str)
        new_bowler_id = int(player_ids_str[new_bowl_idx])
        ...
        afk_user = Match.get_user(afk_bowler_id)
        new_bowler_user = Match.get_user(new_bowler_id)
        batter_user = Match.get_user(batter_id)
        
        await safe_send(client, chat_id,
            f"🚫 {safe_name(afk_user)} eliminated from the match!\n"
            f"🔄 New bowler: {mention(new_bowler_id, safe_name(new_bowler_user))}\n\n"
            f"🎳 {mention(new_bowler_id, safe_name(new_bowler_user))}, bowl now! Check my DM!")
        
        await prompt_bowler_dm(client, match_id, new_bowler_id, batter_id)
        await start_afk_timer(client, match_id, "bowler", new_bowler_id, chat_id)
    
    else:
        # Team mode - remove AFK bowler from bowling order, rotate
        bowl_order = Match.get_team_bowling_order(match)
        bowl_idx = match["team_bowler_index"]
        
        # Remove AFK bowler from bowling order
        if afk_bowler_id in bowl_order:
            bowl_order.remove(afk_bowler_id)
            Match.set_team_bowling_order(match_id, bowl_order)
        
        if len(bowl_order) == 0:
            # No bowlers left - end match, batting team wins
            await safe_send(client, chat_id,
                "❌ No bowlers left! Batting team wins by default!")
            batting_team = match["batting_team"]
            Match.update_match(match_id, state=MatchState.MATCH_OVER)
            await update_team_winner_stats(match_id, batting_team)
            await send_match_awards(client, chat_id, match_id)
            return
        
        # Pick next bowler
        new_bowl_idx = bowl_idx % len(bowl_order)
        new_bowler_id = bowl_order[new_bowl_idx]
        ...
        await safe_send(client, chat_id,
            f"🚫 {safe_name(afk_user)} eliminated from bowling!\n"
            f"🔄 New bowler: {mention(new_bowler_id, safe_name(new_bowler_user))}\n\n"
            f"🎳 {mention(new_bowler_id, safe_name(new_bowler_user))}, bowl now! Check my DM!")
...
async def process_ball_result(client, match_id, chat_id, batter_choice, bowler_choice,
    ...
    if bp:
        ...
        # Update global user stats for sixes and fours
        if runs == 6:
            Match.update_user_stats(batter_id, add_sixes=1)
        elif runs == 4:
            Match.update_user_stats(batter_id, add_fours=1)
    ...
    if bwp and not is_afk_bowler:
        ...
        # Update bowler's global wicket count
        if is_out:
            Match.update_user_stats(bowler_id, add_total_wickets=1)
...
async def update_solo_state(client, match_id, chat_id, runs, is_out, ball_num):
    ...
    if batter_done:
        ...
        await safe_send(client, chat_id,
            f"🏏 Next up: {mention(next_batter_id, bname)} is batting!\n"
            f"🎳 {mention(bowler_uid, bwname)}, bowl first! Check my DM!")

        Match.update_match(match_id, state=MatchState.BOWLER_WAITING)
        await prompt_bowler_dm(client, match_id, bowler_uid, next_batter_id)
        await start_afk_timer(client, match_id, "bowler", bowler_uid, chat_id)
        return

    # Ball done, but batter continues - bowler bowls first for next ball
    bowler_id = match["current_bowler_id"]
    bowler_user = Match.get_user(bowler_id)
    bowler_name = safe_name(bowler_user)
    ...
    # Show mini score - ask bowler to bowl first
    ...
    await safe_send(client, chat_id,
        ...
        f"🎳 {mention(bowler_id, bowler_name)}, bowl next! Check my DM!")

    await prompt_bowler_dm(client, match_id, bowler_id, int(batter_id))
...
async def finish_solo_match(client, match_id, chat_id, scores, player_ids):
    """Determine winner in solo mode with Man of the Match."""
    Match.update_match(match_id, state=MatchState.MATCH_OVER)
    ...
    # Find best batter and best bowler
    players = Match.get_players(match_id)
    ...
    for p in players:
        ...
        runs = scores.get(uid_str, {}).get("runs", 0)
        if runs > best_batter_runs:
            best_batter_runs = runs
            best_batter = p["user_id"]
    
    # Best bowler - most wickets (from match_players table)
    best_bowler = None
    best_bowler_wickets = -1
    best_bowler_runs_given = 9999
    for p in players:
        wickets = p["bowling_wickets"]
        runs_given = p["bowling_runs_given"]
        if wickets > best_bowler_wickets or (wickets == best_bowler_wickets and runs_given < best_bowler_runs_given):
            best_bowler_wickets = wickets
            best_bowler_runs_given = runs_given
            best_bowler = p["user_id"]
    
    # Add awards section
    lines.append("\n🏅 **Match Awards:**")
    
    if best_batter:
        bb_user = Match.get_user(best_batter)
        bb_name = safe_name(bb_user) if bb_user else "Player"
        lines.append(f"🏏 Best Batter: {mention(best_batter, bb_name)} ({best_batter_runs} runs)")
    
    if best_bowler and best_bowler_wickets > 0:
        bw_user = Match.get_user(best_bowler)
        bw_name = safe_name(bw_user) if bw_user else "Player"
        lines.append(f"🎳 Best Bowler: {mention(best_bowler, bw_name)} ({best_bowler_wickets} wickets)")
...
async def update_team_state(client, match_id, chat_id, runs, is_out, ball_num):
    ...
    if is_out and not all_out:
        ...
        if next_bat_idx < len(bat_order):
            ...
            await safe_send(client, chat_id,
                ...
                f"🎳 {mention(bowler_id, bwname)}, bowl! Check my DM!")
            ...
            Match.update_match(match_id, state=MatchState.BOWLER_WAITING)
            await prompt_bowler_dm(client, match_id, bowler_id, new_batter_id)
            await start_afk_timer(client, match_id, "bowler", bowler_id, chat_id)
            return
        else:
            innings_over = True
    ...
    # Continue - next ball - bowler bowls first
    cur_bowler = match["current_bowler_id"]
    ...
    bowler_user = Match.get_user(cur_bowler)
    bname = safe_name(batter_user)
    bwname = safe_name(bowler_user)
    ...
    await safe_send(client, chat_id,
        ...
        f"🎳 {mention(cur_bowler, bwname)}, bowl! Check my DM!")

    await prompt_bowler_dm(client, match_id, cur_bowler, cur_batter)
    await start_afk_timer(client, match_id, "bowler", cur_bowler, chat_id)
...
async def handle_over_change_team(client, match_id, chat_id, batter_id, non_striker_id):
    ...
    bowler_user = Match.get_user(new_bowler_id)
    bname = safe_name(batter_user)
    bwname = safe_name(bowler_user)
    ...
    await safe_send(client, chat_id,
        ...
        f"📊 Score: **{sc}**\n\n"
        f"🏏 {mention(new_batter, bname)} is on strike!\n"
        f"🎳 {mention(new_bowler_id, bwname)}, new over! Bowl first! Check my DM!")

    await prompt_bowler_dm(client, match_id, new_bowler_id, new_batter)
    await start_afk_timer(client, match_id, "bowler", new_bowler_id, chat_id)
...
async def start_innings_2(client, match_id, chat_id):
    ...
    ub = Match.get_user(first_bowler)
    ...
    await safe_send(client, chat_id,
        ...
        f"🎳 {mention(first_bowler, safe_name(ub))}, bowl first! Check my DM!")

    await prompt_bowler_dm(client, match_id, first_bowler, opener1)
    await start_afk_timer(client, match_id, "bowler", first_bowler, chat_id)
...
async def send_match_awards(client, chat_id, match_id):
    """Send best batter and best bowler awards for team mode."""
    all_players = Match.get_players(match_id)
    ...
    best_batter_balls = 0
    for p in all_players:
        if p["batting_runs"] > best_batter_runs:
            best_batter_runs = p["batting_runs"]
            best_batter_balls = p["batting_balls"]
            best_batter = p["user_id"]
    
    # Best bowler - most wickets, then least runs
    best_bowler = None
    best_bowler_wickets = -1
    best_bowler_runs_given = 9999
    for p in all_players:
    ...
    # Build awards message
    awards = ["🏅 **Match Awards:**"]
    
    if best_batter and best_batter_runs > 0:
        bb_user = Match.get_user(best_batter)
        bb_name = safe_name(bb_user) if bb_user else "Player"
        awards.append(f"🏏 Best Batter: {mention(best_batter, bb_name)} ({best_batter_runs} runs off {best_batter_balls} balls)")
    
    if best_bowler and best_bowler_wickets > 0:
        bw_user = Match.get_user(best_bowler)
        bw_name = safe_name(bw_user) if bw_user else "Player"
        overs = f"{best_bowler_runs_given} runs"
        awards.append(f"🎳 Best Bowler: {mention(best_bowler, bw_name)} ({best_bowler_wickets} wickets for {overs})")
...
async def prompt_bowler_dm(client, match_id, bowler_id, batter_id):
    """Send bowling prompt to bowler in DM and notify in group with button."""
    match = Match.get_match_by_id(match_id)
    if not match:
        return
    ...
    # Get bot username for DM button
    global BOT_USERNAME
    if not BOT_USERNAME:
    ...
    # Create DM button
    dm_button = InlineKeyboardMarkup([
        [InlineKeyboardButton("🎳 Open DM to Bowl", url=f"https://t.me/{BOT_USERNAME}")]
    ])
    ...
    if not result:
        ...
        await client.send_message(
            ...
            f"👉 Click the button below and press **START**, then type **1-6**!",
            reply_markup=dm_button,
            parse_mode=ParseMode.MARKDOWN
        )
    else:
        # DM sent successfully - still show button for easy access
        await client.send_message(
            chat_id,
            f"🎳 {mention(bowler_id, bwname)}, it's your turn to bowl!\n"
            f"📩 Check your DM or click the button below:",
            reply_markup=dm_button,
            parse_mode=ParseMode.MARKDOWN
        )
...
async def cmd_startcricket(client, message):
    ...
    keyboard = InlineKeyboardMarkup([
        [
            InlineKeyboardButton("🎯 Solo Mode", callback_data=f"mode_solo_{match_id}"),
            InlineKeyboardButton("👥 Team Mode", callback_data=f"mode_team_{match_id}"),
        ]
    ])
...
@bot.on_callback_query(filters.regex(r"^mode_(solo|team)_(\d+)$"))
async def callback_mode_select(client, callback_query: CallbackQuery):
    ...
    match = Match.get_match_by_id(match_id)
    if not match:
        await callback_query.answer("❌ This lobby no longer exists!", show_alert=True)
        return

    if match["state"] != MatchState.LOBBY:
        await callback_query.answer("❌ Match already started!", show_alert=True)
        return

    # Update mode
    Match.update_match(match_id, mode=mode)
    Match.ensure_user(user.id, safe_name(user), safe_username(user))
    Match.add_player(match_id, user.id, team="A" if mode == "team" else None)
    ...
    if mode == "solo":
        await callback_query.message.edit_text(
            f"🏏 **Solo Mode Lobby** 🏏\n\n"
            f"Host: {mention(user.id, safe_name(user))} ✅ Joined\n\n"
            f"**How to join:**\n"
            f"🎯 Click the button below or type /joinsolo\n\n"
            f"Use /forcestart when everyone has joined!",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🎯 Join Solo", callback_data=f"join_solo_{match_id}")]
            ])
        )
    else:
        await callback_query.message.edit_text(
            f"🏏 **Team Mode Lobby** 🏏\n\n"
            f"Host: {mention(user.id, safe_name(user))} (Team A)\n\n"
            f"**Join a team:**",
            reply_markup=InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("🅰️ Join Team A", callback_data=f"join_teamA_{match_id}"),
                    InlineKeyboardButton("🅱️ Join Team B", callback_data=f"join_teamB_{match_id}"),
                ]
            ])
        )


@bot.on_callback_query(filters.regex(r"^join_solo_(\d+)$"))
async def callback_join_solo(client, callback_query: CallbackQuery):
    """Handle solo join button."""
    match_id = int(callback_query.data.split("_")[2])
    user = callback_query.from_user
    chat_id = callback_query.message.chat.id

    match = Match.get_match_by_id(match_id)
    if not match or match["state"] != MatchState.LOBBY:
        await callback_query.answer("❌ Lobby closed!", show_alert=True)
        return

    Match.ensure_user(user.id, safe_name(user), safe_username(user))
    added = Match.add_player(match_id, user.id, team=None)

    if not added:
        await callback_query.answer("⚠️ You're already in the match!", show_alert=True)
        return

    await callback_query.answer(f"✅ You joined!")
...
@bot.on_callback_query(filters.regex(r"^join_team(A|B)_(\d+)$"))
async def callback_join_team(client, callback_query: CallbackQuery):
    """Handle team join buttons."""
    data = callback_query.data
    team = "A" if "teamA" in data else "B"
    ...
    Match.ensure_user(user.id, safe_name(user), safe_username(user))
    added = Match.add_player(match_id, user.id, team=team)

    if not added:
        await callback_query.answer("⚠️ You're already in the match!", show_alert=True)
        return

    await callback_query.answer(f"✅ Joined Team {team}!")
...
async def start_solo_match(client, match_id, chat_id):
    ...
    Match.update_match(match_id,
        state=MatchState.BOWLER_WAITING,  # Bowler bowls FIRST!
        innings=1,
        current_over=0,
        current_ball=0,
    ...
    await safe_send(client, chat_id,
        ...
        f"🏏 {mention(first_batter, bname)} is batting!\n"
        f"🎳 {mention(bowler_uid, bwname)}, bowl first! Check my DM!")

    # Send DM to bowler first
    await prompt_bowler_dm(client, match_id, bowler_uid, first_batter)
    await start_afk_timer(client, match_id, "bowler", bowler_uid, chat_id)
...
async def cmd_stats(client, message):
    ...
    user_data = Match.get_user(target_user.id)
    
    if not user_data:
        await safe_send(client, message.chat.id,
            f"📊 No stats found for {safe_name(target_user)}!\n"
            f"Play some matches first! 🏏")
        return
    
    name = user_data["display_name"]
    total_runs = user_data["total_runs"]
    matches_played = user_data["matches_played"]
    matches_won = user_data["matches_won"]
    highest = user_data["highest_score"]
    ducks = user_data["ducks"]
    sixes = user_data["sixes"]
    fours = user_data["fours"]
    wickets = user_data["total_wickets"]
    
    # Calculate win rate
    win_rate = (matches_won / matches_played * 100) if matches_played > 0 else 0
    
    stats_text = f"""
📊 **{name}'s Cricket Stats** 📊
...
"""
    ...
    await safe_send(client, message.chat.id, stats_text)
...
async def handle_batter_group(client, message, user_id, number):
    """Handle batter's number in the group - Batter plays AFTER bowler has bowled."""
    chat_id = message.chat.id
    ...
    # Save batter's choice
    Match.update_match(match_id, temp_batter_choice=number)

    batter_user = Match.get_user(user_id)
    bname = safe_name(batter_user)

    await safe_send(client, chat_id, f"🏏 {mention(user_id, bname)} played **{number}**! ✅")

    # Get bowler's choice (already stored secretly)
    bowler_choice = match["temp_bowler_choice"]

    # Process the ball - show result!
    await process_ball_result(client, match_id, chat_id, number, bowler_choice)


async def handle_bowler_dm(client, message, user_id, number):
    """Handle bowler's number in DM - Bowler bowls FIRST (secretly)."""
    ...
    # Save bowler choice secretly, switch to BATTER_WAITING
    Match.update_match(match_id,
    ...
    await safe_send_dm(client, user_id,
        f"🎳 You bowled **{number}**! ✅\n"
        f"⏳ Waiting for batter to play...")

    # Now ask batter to play in group
    batter_id = match["current_batter_id"]
    batter_user = Match.get_user(batter_id)
    bname = safe_name(batter_user)

    await safe_send(client, chat_id,
        f"🎳 Bowler has bowled! 🤫 (Number hidden)\n\n"
        f"🏏 {mention(batter_id, bname)}, your turn! Type **1-6** in the group!")

    await start_afk_timer(client, match_id, "batter", batter_id, chat_id)
