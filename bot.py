...
from animations import send_gif
...
async def process_ball_result(client, match_id, chat_id, batter_choice, bowler_choice,
    ...
    await safe_send(client, chat_id, result_text)
...
async def update_solo_state(client, match_id, chat_id, runs, is_out, ball_num):
    ...
    if batter_done:
        ...
        await safe_send(client, chat_id,
            ...
            f"🎳 {mention(bowler_uid, bwname)} is bowling!\n\n"
            f"🏏 {mention(next_batter_id, bname)}, type a number **1-6** in the group!")
    ...
    await safe_send(client, chat_id,
        f"📊 {mention(int(batter_id), safe_name(Match.get_user(int(batter_id))))}:"
        f" **{total_runs}** ({total_balls}b) | Over: {match['current_over']}.{ball_num}\n\n"
        f"🏏 Type your next number **1-6**!")
...
async def handle_over_change_team(client, match_id, chat_id, batter_id, non_striker_id):
    ...
    await safe_send(client, chat_id,
        ...
        f"📊 Score: **{sc}**\n\n"
        f"🎳 New bowler: {mention(new_bowler_id, bwname)}\n"
        f"🏏 On strike: {mention(new_batter, bname)}\n\n"
        f"Type **1-6** to play!")
...
async def start_innings_2(client, match_id, chat_id):
    ...
    await safe_send(client, chat_id,
        ...
        f"🎳 Bowling: {mention(first_bowler, safe_name(ub))}\n\n"
...
async def prompt_bowler_dm(client, match_id, bowler_id, batter_id):
    """Send bowling prompt to bowler in DM."""
    match = Match.get_match_by_id(match_id)
    if not match:
        return
    ...
    result = await safe_send_dm(client, bowler_id, msg)
    if not result:
        chat_id = match["chat_id"]
        bowler_user = Match.get_user(bowler_id)
        bwname = safe_name(bowler_user)
        await safe_send(client, chat_id,
            f"⚠️ {mention(bowler_id, bwname)}, I can't DM you!\n"
            f"Please start me in private first: press /start in my DM, then I'll send you the bowling prompt.")
...
async def start_solo_match(client, match_id, chat_id):
    ...
    scores = {}
    first_batter = int(player_ids[0])
    scores[player_ids[0]] = {"runs": 0, "balls": 0, "is_out": False, "batting": True}

    # Pick first bowler (not the batter)
    bowler_uid = pick_solo_bowler(player_ids, 0, -1)
    ...
    Match.update_match(match_id,
        ...
        solo_bowling_index=player_ids.index(str(bowler_uid)),
        temp_batter_choice=0,
        temp_bowler_choice=0,
        over_balls_json="[]",
        bowler_last_ball=0,
    ...
    order_text = "\n".join(
        f"  {i+1}. {safe_name(Match.get_user(int(uid)))}"
    ...
    await safe_send(client, chat_id,
        ...
        f"🎳 {mention(bowler_uid, bwname)} is bowling!\n\n"
        f"🏏 {mention(first_batter, bname)}, type **1-6** in the group!")
...
async def handle_batter_group(client, message, user_id, number):
    ...
    await safe_send(client, chat_id,
        ...
        f"⏳ Waiting for the bowler to bowl in DM...")
