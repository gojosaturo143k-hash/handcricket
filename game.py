...
from rules import COMMENTARY, SLEDGES
...
class Match:
    ...
    def add_player(self, user_id, team='solo'):
        ...
        self.db.execute(
            "INSERT INTO match_players (match_id, telegram_id, team, join_order) VALUES (?, ?, ?, ?)",
            (self.match_id, user_id, team, count + 1)
        )
        self.db.commit()
        return True
    ...
    def get_batting_team_players(self):
        m = self.get()
        if not m: return []
        team = 'a' if m['current_innings'] == 1 else 'b'
        return self.get_players(team=team)

    def get_bowling_team_players(self):
        m = self.get()
        if not m: return []
        team = 'b' if m['current_innings'] == 1 else 'a'
        return self.get_players(team=team)
    ...
    def get_current_batter(self):
        return self.db.execute(
            "SELECT * FROM match_players WHERE match_id = ? AND is_batting = 1", (self.match_id,)
        ).fetchone()
    ...
    def get_innings_deliveries(self, innings):
        return self.db.execute(
            "SELECT * FROM deliveries WHERE match_id = ? AND innings = ? ORDER BY id DESC", 
            (self.match_id, innings)
        ).fetchall()

    def get_last_deliveries(self, over_num, innings, limit=3):
        return self.db.execute(
    ...
    def reset_strike(self):
        self.db.execute("UPDATE match_players SET is_batting = 0, is_bowling = 0 WHERE match_id = ?", (self.match_id,))
        self.db.commit()
